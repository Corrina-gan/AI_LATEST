...
def _movie_card_html(row: pd.Series, score_columns: list[str]) -> str:
    ...
    count = row.get("n_ratings") #Number of rating
    if pd.notna(avg):
...
def render_hybrid_recs(
    *,
    ...
    render_recommendation_cards(display, []) #Display movie cards
...
def render_hybrid_weight(
    *,
    ...
    if loaded is not None:
        ...
        source_counts = display["blend_source"].value_counts() #counts how many recommended movies fall into each category
        st.html( #Display the counts
            _stat_tiles_html(
                [
                    (
                        "Content-leaning",
    ...
    *,
    ...
    if stored_query or stored_genres:
        ...
        if matches.empty:
            ...
            if st.button("Show predicted rating for this user", type="primary"): #Predict the rating
...
def render_visualization(data_sig: str, ratings, movies) -> None:
    ...
    st.pyplot(data_visualization.make_top_tags(tags), clear_figure=True, width="content") #Shows the 20 most commonly used movie tags
    ...
    pareto_fig, pct_to_80, pct_le5 = data_visualization.make_long_tail_pareto(ratings) #Long-tail popularity, analyses movie popularity
    ...
    st.pyplot(
        data_visualization.make_genre_decade_heatmap(ratings, movies), #Shows average ratings across genres, decades
        clear_figure=True,
        width="content",
    )
...
def main() -> None:
    ...
    if getattr(hybrid_model.content_model, "movies", None) is None: #checks whether the Hybrid model's Content-Based model has movie data
